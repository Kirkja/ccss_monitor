...
app.controller("SettingsController", function($scope, Api, $http, $window) {
    ...
    $scope.$watch('currentCatalogID', function(newval, oldval) {
     ...
     $scope.$watch('currentNote', function(newval, oldval) {
        ...
        if (oldval === "") {
         ...
         $scope.noteStatus = "Needs to be saved";
        }
     });
    ...
    $scope.showSelected = function(sel) {
        ...
        if ($scope.selected.children.length === 0) {
            ...
            $scope.collectorID              = $scope.selected.collectorID;
   ...
   //
    ...
    $scope.addSCR = function() {
        ...
        if ($scope.collectorID === "") {
            alert("A collectorID error occured.")
            return;
        }         
    ...
    $scope.delSCR = function() {
        ...
        if (checked.length > 0) {
            ...
            .success(function(response, status, headers, config) {            
                $scope.rdf2 = response.data;
                //--had to comment this as it was causing an error in
                // angular; $rootScope:inprog 
                //$scope.$apply();
            })
            .error(function(response, status, headers, config) {
                $scope.status = "errors";
    ...
    $scope.debug = function() {
        alert("Debug Values: " 
            + "\nAccount ID:"  + $scope.user.activeID
            + "\nCollector ID:" + $scope.collectorID
            + "\nBlock ID:"  + $scope.currentBlockID
            + "\nSample ID:"  + $scope.currentSampleID
            + "\nImage ID:"  + $scope.currentImageID 
            + "\nCatalog ID:"  + $scope.currentCatalogID
            + "\nGroup ID:"  + $scope.currentGroupingID
            + "\nNote ID:" + $scope.currentNoteID 
